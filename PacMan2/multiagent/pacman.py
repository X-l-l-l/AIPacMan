...
class PacmanRules:
    ...
    def applyAction(state, action):
        ...
        if next in state.getHoles():
            if state.data._jumpEaten == None:
                ...
                if walls[next[0]][next[1]]:
                    ...
                    pacmanState.configuration = pacmanState.configuration.generateSuccessor(
                        vector)
                    PacmanRules.consume(state.getPacmanPosition(), state)
    ...
    def consume(position, state):
        ...
        # Eat jump
        if (position == state.getJump()):
            state.data.jump = None
            state.data._jumpEaten = position
