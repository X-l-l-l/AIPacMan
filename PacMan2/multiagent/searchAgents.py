        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                if (nextx, nexty) in self.corners:
                    currentState[self.corners.index((nextx, nexty))] = True

                nextState = ((nextx, nexty), currentState)
                cost = 1
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    return 0 if len(foodGrid.asList()) == 0 else \
        max([mazeDistance(position, item, problem.startingGameState) for item in foodGrid.asList()])
